...
export const CodeBlock: React.FC<CodeBlockProps> = (props) => {
    return (
        <div className="my-8 border rounded-md section bg-slate-50 border-slate-200 dark:bg-slate-900 dark:border-slate-600">
            {props.label && (
                <div className="px-4 py-1 border-b border-slate-200 bg-slate-200 dark:border-slate-600 dark:bg-slate-800">
                    <span className="font-mono text-xs font-bold text-slate-500 dark:text-slate-200">{props.label}</span>
                </div>
            )}
